...
def update_item(request,pk):
    ...
    if request.method == "POST":
        ...
        #show  whats wrong if it fails:
            ...
            return render(request,"monsterhunter/monster_form.html",{"form":form,"is_edit":True})
#2.find all the drops for that monster and get its pk:
def monster_drops(request,pk):
    #store the monster drops inside a variable:
    drops = get_object_or_404(MonsterItemDrop.objects.filter(monster=monster),pk=pk)
    return render(request,"monsterhunter/drop_list.html",{"drops":drops})
